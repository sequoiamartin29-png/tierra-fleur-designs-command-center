...
function SectionView({ section, vm, onPlant, onPhoto, onAddOnInterest, onApproval }) {
  ...
  return <section className={`presentation-section section-${key}`} data-section-key={key}>
    ...
    {key === 'recommended-design' && <>{recommended ? <div className="recommended-layout"><div><span>Recommended direction</span><h3>{recommended.name}</h3><p>{recommended.description}</p>{recommended.designGoals && <blockquote>{recommended.designGoals}</blockquote>}<ConceptCanvasPreview concept={recommended} photo={vm.photos.find(item => item.photoId === recommended.heroPhotoId) || vm.photos.find(item => item.photoId === recommended.canvas?.basePhotoId) || featured} />{recommended.showLegend && recommended.canvas?.placements?.length > 0 && <div className="canvas-client-legend">{[...new Set(recommended.canvas.placements.map(item => item.label).filter(Boolean))].map(label => <span key={label}>{label}</span>)}</div>}</div></div> : <PresentationEmpty title="Recommendation to come" text="Choose a client-visible concept and mark it recommended in the builder." />}</>}
